...
//	}
	...
	public static void toggleSoundsSettings() {
		...
		GaUtils.logEvent(AnalyticsEvents.CATEGORY_SOUNDS, AnalyticsEvents.ACTION_POWER + " " + (Settings.getInstance().isSoundOn() ? "on" : "off"));
	}
	...
	public static Comics create(Context context, BaseState state) {
		...
		if (descriptor == null)
			return null;
		Comics comics = JsonSerializer.fromJson(FileUtils.readStream(descriptor.getData()), Comics.class);
		if (comics != null)
