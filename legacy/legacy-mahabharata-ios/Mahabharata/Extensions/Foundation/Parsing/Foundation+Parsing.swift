//
//  Foundation+Parsing.swift
//  Foundation+Parsing
//
//  Created by Vasiliy Ursu on 20 Jun 2018.
//	Updated by Alexey Gushin on 12 Dec 2019.
...
//      13 Dec 2019. Alexey Gushin:
//			* Added hanling nil value by parsing arrays + added default value in array method's
//
...
/*
extension Decodable {
	static func parse(_ data: Any?, optional val: Decodable? = nil, formatter: NumberFormatter? = nil) throws -> Decodable? {
		guard let data = data else {
			return val
		}
		if let data = data as? Decodable {
			return data
			...
			// TODO: parse decodable from data
	...
	static func array(_ data: Any?, optional val: [Decodable]? = nil) throws -> [Decodable]? {
		if data == nil || data is NSNull { return val }
		
		guard let array = data as? [Any] else {
			throw "Field \"data\" is not array type"
		}
		return array.compactMap { try! parse($0) }
	}
}
*/
...
extension Int {
	...
	*/
	...
	static func array(_ data: Any?, optional val: [Int]? = nil) throws -> [Int]? {
		if data == nil || data is NSNull { return val }
...
extension Double {
	...
	static func array(_ data: Any?, optional val: [Double]? = nil) throws -> [Double]? {
		if data == nil || data is NSNull { return val }
...
extension Float {
	...
	static func array(_ data: Any?, optional val: [Float]? = nil) throws -> [Float]? {
		if data == nil || data is NSNull { return val }
...
extension String {
	...
	static func array(_ data: Any?, optional val: [String]? = nil) throws -> [String]? {
		if data == nil || data is NSNull { return val }
...
extension Array {
	...
	static func array<T>(_ data: Any?, optional val: [[T]]? = nil) throws -> [[T]]? {
		if data == nil || data is NSNull { return val }
...
extension Dictionary {
	...
	static func array(_ data: Any?, optional val: [[String: Any]]? = nil) throws -> [[String: Any]]? {
		if data == nil || data is NSNull { return val }
...
extension NSNumber {
	...
	static func array(_ data: Any?, optional val: [NSNumber]? = nil) throws -> [NSNumber]? {
		if data == nil || data is NSNull { return val }
	...
	static func parse(_ data: Any?, optional val: Date? = nil) throws -> Date? {
		...
		if data is NSNull {
			...
			if CharacterSet.decimalDigits.isSuperset(of: CharacterSet(charactersIn: data)) {
				...
				return Date.parseIso8601(data) ?? val
			}
		}
		return val
	}
	
	static func array(_ data: Any?, optional val: [Date]? = nil) throws -> [Date]? {
		if data == nil || data is NSNull { return val }
...
extension Bool {
	...
	static func array(_ data: Any?, optional val: [Bool]? = nil) throws -> [Bool]? {
		if data == nil || data is NSNull { return val }
...
extension Dictionary {
	/// Formatting JSON see also https://github.com/SwiftyJSON/SwiftyJSON
	func toJSON(formatted: Bool) -> String? {
		guard JSONSerialization.isValidJSONObject(self) else { return nil }
		let options: JSONSerialization.WritingOptions = formatted ? .prettyPrinted : .init(rawValue: .zero)
...
extension Dictionary where Key: Encodable, Value: Encodable {
	func toJSON(formatted: Bool) -> String {
		...
		encoder.outputFormatting = formatted ? .prettyPrinted : .init(rawValue: .zero)
